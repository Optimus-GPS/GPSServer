<? 
	...
	{			
		...
		// get records number
		$q = "SELECT * FROM `gs_user_cmd` WHERE `user_id`='".$user_id."' or `user_id` = '1' group by cmd";
		$r = mysqli_query($ms, $q);
		$count = mysqli_num_rows($r);
	...
	{
		$q = "SELECT * FROM `gs_user_cmd` WHERE `user_id`='".$user_id."' or `user_id` = '1' ORDER BY `cmd_id` ASC";
		$r = mysqli_query($ms, $q);
