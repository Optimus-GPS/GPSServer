		...
		while($od = mysqli_fetch_array($r))
		{
			...
			{			
				...
				{					
					...
					if (($ed['type'] == 'dismount') && ($loc['event'] == 'removing'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					...
					if (($ed['type'] == 'haccel') && ($loc['event'] == 'hardAcceleration'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'hbrake') && ($loc['event'] == 'hardBraking'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'hcorn') && ($loc['event'] == 'hardCornering'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'pwrcut') && ($loc['event'] == 'powerCut'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'gpsantcut') && ($loc['event'] == 'gpsAntennaCut'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'lowdc') && ($loc['event'] == 'lowPower'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
					
					if (($ed['type'] == 'lowbat') && ($loc['event'] == 'lowBattery'))
					{
						event_tracker($ed,$ud,$od,$loc);
					}
...
// echo "3";				
	...
	function event_sensor($ed,$ud,$od,$loc,$loc_prev)
	{
		...
		{			
			...
			if (($sc[$i]['cn'] == 'grp') || ($sc[$i]['cn'] == 'lwp'))
			{
				...
				if (($sc[$i]['cn'] == 'grp') && ($value > $value_prev))
				{
					...
					if ($sensor['type'] == 'fuel' || $sensor['type'] == 'fuelsumup'){
						if ($value_diff > $sc[$i]['val']) $cn = true;	
					}else{
		...
		if ($condition)
		{
			if (get_event_status($ed['event_id'], $loc['imei']) == -1)
			{
				...
				// add event desc to event data array
				$ed['event_desc'] = $ed['name'];
				event_notify($ed,$ud,$od,$loc,$value_diff);
			}			
		}
		else
		{
	...
	function event_notify($ed,$ud,$od,$loc,$value_diff=false)
	{
		global $ms, $gsValues;
		...
		if (checkUserUsage($ed['user_id'], 'email'))
		{
			...
			{			
				...
				$template = event_notify_template('email',$ed,$ud,$od,$loc,$value_diff);
		...
		if (checkUserUsage($ed['user_id'], 'sms'))
		{
			if (($ed['notify_sms'] == 'true') && ($ed['notify_sms_number'] != ''))
			{
				...
				$template = event_notify_template('sms',$ed,$ud,$od,$loc,$value_diff);
	...
	function event_notify_template($type,$ed,$ud,$od,$loc,$value_diff=false)
	{
		global $ms, $la;
		...
		foreach ($template as $key => $value)
		{
			...
			$value = str_replace("%VAL_DIFF%", $value_diff, $value);
