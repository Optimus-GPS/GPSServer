...
$ch = curl_init();
$SERVER_COMTAVO = 'http://localhost';
curl_setopt($ch, CURLOPT_URL, $SERVER_COMTAVO."/server/http/gps2.php?" . "deviceid=" . $_GET["deviceid"] ."&protocol=".$_GET["protocol"]."&attributes=".$_GET["attributes"]."&fixtime=".$_GET["fixtime"]."&longitude=".$_GET["longitude"]."&latitude=".$_GET["latitude"]."&altitude=".$_GET["fixtime"]."&speed=".$_GET["speed"]."&course=".$_GET["course"]."&valid=".$_GET["valid"]);
