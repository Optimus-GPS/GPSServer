    ...
    switch (e) {
        ...
        case "save":
            ...
            var t = document.getElementById("dialog_maintenance_service_name").value,
                ...
                h = document.getElementById("dialog_maintenance_service_days_left").checked,
            ...
            if (parseFloat(g) <= parseFloat(b) && 1 == h) {
                notifyBox("error", la.ERROR, la.INTERVAL_VALUE_SHOULD_BE_GREATER_THAN_LEFT_VALUE);
                break
            }
            var f = {
        ...
        if (i.clearGridData(!0), "" != a) {
            ...
            switch (e) {
                case "object":
                    ...
                    var h = objectsData[t].service;
                    for (var b in h) "true" == h[b].data_list && o.push({
...
function cmdGPRSTemplateList() {
    ...
    if (multiselectIsSelected(o)) {
        var i = "",
			...
			// alert("device = "+settingsObjectData[s[0]].device); //GRivera
			// alert("protocol = "+settingsObjectData[s[0]].protocol); //GRivera
        for (a = 0; a < s.length; a += 1)
			// alert("protocol = "+settingsObjectData[s[0]].protocol); //GRivera
            if (null != settingsObjectData[s[0]] && null != settingsObjectData[s[a]] && (i = settingsObjectData[s[0]].device) != settingsObjectData[s[a]].device) {
				// alert("Protocol = "+settingsObjectData[s[0]].protocol);
                i = "";
                break
            } if ("" != i) {
            ...
            for (var n in r.appendTo(e), e.options.add(new Option(la.CUSTOM, "")), cmdData.cmd_templates) {
				...
				// alert("_.name= " + _.name );
				 // alert("_= " + _.protocol.toLowerCase().trim());
				 // alert("i = " + i.toLowerCase().trim());
				 if(_.protocol.toLowerCase().trim() == i.toLowerCase().trim()){
                "gprs" == _.gateway && (_.protocol.toLowerCase().trim() == i.toLowerCase().trim() ? d.push({
                    name: _.name,
                    key: n
...
function cmdSMSTemplateList() {
    ...
    if (multiselectIsSelected(o)) {
        var i = "",
            s = (o = multiselectGetValues(o)).split(",");
			// alert(settingsObjectData[s[0]].device);
        for (a = 0; a < s.length; a += 1)
            if (null != settingsObjectData[s[0]] && null != settingsObjectData[s[a]] && (i = settingsObjectData[s[0]].device) != settingsObjectData[s[a]].device) {
                // alert(settingsObjectData[s[0]].protocol);
				i = "";
                break
            } if ("" != i) {
            var n = new Array,
            ...
            for (var d in l.appendTo(e), e.options.add(new Option(la.CUSTOM, "")), cmdData.cmd_templates) {
                var r = cmdData.cmd_templates[d];
				 // alert("r= " + r.protocol.toLowerCase());
				 // alert("i = " + i.toLowerCase());
				  // if(_.protocol.toLowerCase().trim() == i.toLowerCase().trim()){				 
...
function cmdTemplateScheduleList() {
	
	var gateway_opt = document.getElementById("dialog_cmd_schedule_cmd_gateway").value;
	// alert(gateway_opt);
	if (gateway_opt == "gprs"){
    for (var e = document.getElementById("dialog_cmd_schedule_template_list"), t = e.getElementsByTagName("optgroup"), a = t.length - 1; a >= 0; a--) e.removeChild(t[a]);
    e.options.length = 0;
    ...
    $.ajax({
        ...
        success: function(e) {
            ...
            geocoderGetAddress(t, a, function(i) {
                ...
                var h = p + r;
                addPopupToMap(t, a, [0, 0], p += "</table>", h += "</table>"), map.panTo({
                    lat: t,
                    lng: a
...
function initGui() {
    $("#map_action_menu").menu({
        ...
        resizable: !1,
        dialogClass: "dialog-billing-titlebar"
    }), initSelectList("object_device_list"), $("#side_panel_objects_dragbar").mousedown(function(e) {
...
function initSelectList(e) {
    switch (e) {
		...
		// case "object_device_list":
            // var t = document.getElementById("dialog_object_add_device");
            // t.options.length = 0;
...
function historyLoadRouteDblClick(a) {
	if (idSeg != a){
		...
		if (utilsCheckPrivileges("history")) {
			...
			var e = document.getElementById("side_panel_history_object_list").value = a,
				t = document.getElementById("side_panel_history_object_list").options[document.getElementById("side_panel_history_object_list").selectedIndex].text,
				a = document.getElementById("side_panel_history_date_from").value + " " + document.getElementById("side_panel_history_hour_from").value + ":" + document.getElementById("side_panel_history_minute_from").value + ":00",
				o = document.getElementById("side_panel_history_date_to").value + " " + document.getElementById("side_panel_history_hour_to").value + ":" + document.getElementById("side_panel_history_minute_to").value + ":00",
				i = document.getElementById("side_panel_history_stop_duration").value;
    ...
    if (datalistShowData("route", o, c), t) {
        ...
        geocoderGetAddress(s, i, function(t) {
            ...
            for (var y in v) {
                ...
                if ("true" == h.popup)
                    if ("fuelsumup" == h.type) {
                        var b = getSensorValueFuelLevelSumUp(o, _, h);
                        u += "<tr><td><strong>" + h.name + ":</strong></td><td>" + b.value_full + "</td></tr>"
                    } else {
                        b = getSensorValue(_, h);
                        u += "<tr><td><strong>" + h.name + ":</strong></td><td>" + b.value_full + "</td></tr>"
                    }
            }
...
function historyRouteShowStop(e) {
    ...
    geocoderGetAddress(i, o, function(e) {
        ...
        for (var y in v) {
            ...
            if ("true" == h.popup)
                if ("fuelsumup" == h.type) {
                    var b = getSensorValueFuelLevelSumUp(a, _, h);
                    u += "<tr><td><strong>" + h.name + ":</strong></td><td>" + b.value_full + "</td></tr>"
                } else {
                    b = getSensorValue(_, h);
                    u += "<tr><td><strong>" + h.name + ":</strong></td><td>" + b.value_full + "</td></tr>"
    ...
    switch (e) {
        ...
        case "save":
            ...
            var _ = document.getElementById("dialog_report_name").value,
                ...
                h = document.getElementById("dialog_report_speed_limit").value,
            ...
            if (("overspeed" == c || "underspeed" == c) && "" == h) return void notifyBox("error", la.ERROR, la.SPEED_LIMIT_CANT_BE_EMPTY);
    ...
    switch (e) {
        ...
        case "save":
            ...
            if ("route_in" != _ || "route_out" != _) {
                ...
                if ("route_in" == _ || "route_out" == _) {
                    if (h = "off", !multiselectIsSelected(b)) {
                        notifyBox("error", la.ERROR, la.AT_LEAST_ONE_ROUTE_SELECTED);
                        break
                    }
                } else h = document.getElementById("dialog_settings_event_route_trigger").value;
...
function settingsSave() {
    if (utilsCheckPrivileges("viewer")) {
        ...
        "" == y && (y = !1);
        var v = document.getElementById("settings_main_language").value,
            h = document.getElementById("settings_main_distance_unit").value;
        h += "," + document.getElementById("settings_main_capacity_unit").value, h += "," + document.getElementById("settings_main_temperature_unit").value;
        ...
        var O = document.getElementById("settings_main_old_password").value,
            ...
            T = document.getElementById("settings_main_new_password_rep").value;
        if (O.length > 0) {
            ...
            if (j != T) return void notifyBox("error", la.ERROR, la.REPEATED_PASSWORD_IS_INCORRECT)
        }
        var R = {
...
function settingsObjectSensorProperties(a) {
    ...
    switch (a) {
        default:
			...
			"percentage" == h ? (document.getElementById("dialog_settings_object_sensor_lv").value = settingsObjectData[b].sensors[c].lv, document.getElementById("dialog_settings_object_sensor_hv").value = settingsObjectData[b].sensors[c].hv) : (document.getElementById("dialog_settings_object_sensor_lv").value = "", document.getElementById("dialog_settings_object_sensor_hv").value = ""), "acc" == settingsObjectData[b].sensors[c].type ? document.getElementById("dialog_settings_object_sensor_acc_ignore").checked = !1 : document.getElementById("dialog_settings_object_sensor_acc_ignore").checked = strToBoolean(settingsObjectData[b].sensors[c].acc_ignore), 
			...
			settingsEditData.sensor_dictionary = settingsObjectData[b].sensors[c].dictionary.slice(0), 
        ...
        case "save":
            ...
            if ("" != formula) {
                formula = formula.toLowerCase();//, formula = formula.replace(/[^-()\d/*+.x]/g, ""); //GRivera Se comento la linea para que no omitiera esos caracteres
    ...
    switch (e) {
        ...
        case "save":
            ...
            var o = document.getElementById("dialog_settings_object_service_name").value,
                ...
                h = document.getElementById("dialog_settings_object_service_engh_left_num").value,
            ...
            if (parseFloat(_) <= parseFloat(h) && 1 == v) {
                notifyBox("error", la.ERROR, la.INTERVAL_VALUE_SHOULD_BE_GREATER_THAN_LEFT_VALUE);
                break
            }
...
//Seguimiento 
function utilsFollowObjectHistory(e, t) {
    if ("" != objectsData[e].data) {
        var a = document.getElementById("map_layer").value,
            // o = "func/fn_object.follow.php?imei=" + e + "&map_layer=" + a;
			 now = new Date(),
			 today = now.toISOString().slice(0, 10) + " 00:00:00";
			let now2 = new Date();
			now.setDate(now.getDate() + 1);
			var twomorrow = now.toISOString().slice(0, 10) + " 00:00:00";			
			o = "func/fn_history.php?cmd=load_msg_list&imei=" + e + "&dtf=" + today + "&dtt=" + twomorrow;
